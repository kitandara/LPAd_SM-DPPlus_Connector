...
public class HttpRSPClient {
  ...
  static {
    setTrustAllSslCerts(); // Trust all certs..
  }

  private static void setTrustAllSslCerts() {
    ...
    try {
      ...
      LogStub.getInstance().logDebug(LOG, LogStub.getInstance().getTag() + " Set SSL to trust all certs successfully: ");
    } catch (Exception e) {
      LogStub.getInstance().logDebug(LOG, LogStub.getInstance().getTag() + "Error setting SSL config: " + e.getMessage());
    }
  }
