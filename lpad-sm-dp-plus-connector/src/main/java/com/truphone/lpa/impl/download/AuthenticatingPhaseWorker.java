...
public class AuthenticatingPhaseWorker {
  ...
  private AuthenticateClientResp getAuthenticateClientResponse(InitialAuthenticationKeys initialAuthenticationKeys, String encodedAuthenticateServerResponse) {
    ...
    AuthenticateClientResp authenticateClientResp = es9Module.authenticateClient(initialAuthenticationKeys.getTransactionId(), encodedAuthenticateServerResponse,
        initialAuthenticationKeys.getEuiccConfiguredAddress());
  ...
  public AuthenticateClientRespEs11 getAuthenticateClientResponseES11(InitialAuthenticationKeys initialAuthenticationKeys, String encodedAuthenticateServerResponse) {

    if (LogStub.getInstance().isDebugEnabled()) {
      LogStub.getInstance().logDebug(LOG, LogStub.getInstance().getTag() + " - Authenticate client with SM-DS with initialAuthenticationKeys: " +
          initialAuthenticationKeys +
          " encodedAuthenticateServerResponse: " +
          encodedAuthenticateServerResponse);
    }

    AuthenticateClientRespEs11 authenticateClientResp = es9Module.authenticateClientEs11(initialAuthenticationKeys.getTransactionId(), encodedAuthenticateServerResponse,
        initialAuthenticationKeys.getEuiccConfiguredAddress());

    if (LogStub.getInstance().isDebugEnabled()) {
      LogStub.getInstance().logDebug(LOG, LogStub.getInstance().getTag() + " - Authenticate client with SM-DS - Response : " + authenticateClientResp);
    }
